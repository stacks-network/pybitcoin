...
_reference_info = {
    ...
    'bin_private_key': ("\xc4\xbb\xcb\x1f\xbe\xc9\x9de\xbfY\xd8\\\x8c\xb6.\xe2"
                        "\xdb\x96?\x0f\xe1\x06\xf4\x83\xd9\xaf\xa7;\xd4\xe3"
                        "\x9a\x8a"),
    'hex_private_key': ("c4bbcb1fbec99d65bf59d85c8cb62ee2db963f0fe106f483d9afa"
                        "73bd4e39a8a"),
    'hex_public_key': ("0478d430274f8c5ec1321338151e9f27f4c676a008bdf8638d07c0"
                       "b6be9ab35c71a1518063243acd4dfe96b66e3f2ec8013c8e072cd0"
                       "9b3834a19f81f659cc3455"),
...
class BitcoinCompressedPublicKeyTest(unittest.TestCase):
    def setUp(self):
        self.reference = {
            'hex_public_key': ("02068fd9d47283fb310e6dfb66b141dd78fbabc76d073d"
                               "48cddc770ffb2bd262d7"),
            'bin_public_key': ("\x02\x06\x8f\xd9\xd4r\x83\xfb1\x0em\xfbf\xb1A"
                               "\xddx\xfb\xab\xc7m\x07=H\xcd\xdcw\x0f\xfb+"
                               "\xd2b\xd7"),
            'hex_hash160': '25488b0d3bb770d6e0ef07e1f19d33ab59931dee',
            'bin_hash160': ("%H\x8b\r;\xb7p\xd6\xe0\xef\x07\xe1\xf1\x9d3\xabY"
                            "\x93\x1d\xee"),
    ...
    def test_address(self):
        self.assertEqual(self.public_key.address(),
...
class BitcoinPublicKeyCreationTest(unittest.TestCase):
    ...
    def test_create_pubkey_from_hex_uncompressed_format(self):
        public_key_string = ("04068fd9d47283fb310e6dfb66b141dd78fbabc76d073d48"
                             "cddc770ffb2bd262d7b2832f87f683100b89c2e95314deee"
                             "acbc6409af1e36c3ae3fd8c5f2f243cfec")
    ...
    def test_create_pubkey_from_bin_uncompressed_format(self):
        public_key_string = ("\x04\x06\x8f\xd9\xd4r\x83\xfb1\x0em\xfbf\xb1A"
                             "\xddx\xfb\xab\xc7m\x07=H\xcd\xdcw\x0f\xfb+\xd2b"
                             "\xd7\xb2\x83/\x87\xf6\x83\x10\x0b\x89\xc2\xe9S"
                             "\x14\xde\xee\xac\xbcd\t\xaf\x1e6\xc3\xae?\xd8"
                             "\xc5\xf2\xf2C\xcf\xec")
        self.assertEqual(self.address_uncompressed,
                         BitcoinPublicKey(public_key_string).address())

    def test_create_pubkey_from_hex_ecdsa_format(self):
        public_key_string = ("068fd9d47283fb310e6dfb66b141dd78fbabc76d073d48cd"
                             "dc770ffb2bd262d7b2832f87f683100b89c2e95314deeeac"
                             "bc6409af1e36c3ae3fd8c5f2f243cfec")
        self.assertEqual(self.address_uncompressed,
                         BitcoinPublicKey(public_key_string).address())

    def test_create_pubkey_from_bin_ecdsa_format(self):
        public_key_string = (
            "\x06\x8f\xd9\xd4r\x83\xfb1\x0em\xfbf\xb1A\xddx\xfb\xab\xc7m\x07=H"
            "\xcd\xdcw\x0f\xfb+\xd2b\xd7\xb2\x83/\x87\xf6\x83\x10\x0b\x89\xc2"
            "\xe9S\x14\xde\xee\xac\xbcd\t\xaf\x1e6\xc3\xae?\xd8\xc5\xf2\xf2C"
            "\xcf\xec")
        self.assertEqual(self.address_uncompressed,
                         BitcoinPublicKey(public_key_string).address())

    def test_create_pubkey_from_hex_compressed_format(self):
        public_key_string = ("02068fd9d47283fb310e6dfb66b141dd78fbabc76d073d48"
                             "cddc770ffb2bd262d7")
    ...
    def test_create_pubkey_from_bin_compressed_format(self):
        public_key_string = (
            "\x02\x06\x8f\xd9\xd4r\x83\xfb1\x0em\xfbf\xb1A\xddx\xfb\xab\xc7m"
            "\x07=H\xcd\xdcw\x0f\xfb+\xd2b\xd7")
        self.assertEqual(self.address_compressed,
                         BitcoinPublicKey(public_key_string).address())
...
class BitcoinBrainWalletKeypairTest(BitcoinKeypairTest):
    ...
    def test_passphrase(self):
        self.assertEqual(self.keypair.passphrase(),
                         self.reference['passphrase'])
...
class SequentialWalletTest(unittest.TestCase):
    ...
    def test_bitcoin_keypairs(self):
        ...
        self.assertEqual(
            self.reference['bitcoin_keypair_1']['address'],
...
class ServicesGetUnspentsTest(unittest.TestCase):
    def setUp(self):
        ...
        self.unspents_dict = {
            ...
            {
                'script_hex': ("76a9148eac3d867e1f92f47da40217c3cbd3d75d05701"
                               "388ac"),
                'output_index': 1,
                'transaction_hash': ("691d1645dc6f9431fe2ef414aaa88887efb5fed"
                                     "9354bf53ed01349595bf725ed"),
                'confirmations': 6146,
                'value': 55366270
            }
        }
    ...
    def compare_unspents(self, unspents):
        for unspent in unspents:
            if unspent['transaction_hash'] in self.unspents_dict:
                ...
                self.assertEqual(
                    unspent['value'], ref_unspent['value'])
            else:
                continue
...
class SendNamecoinTransactionTest(unittest.TestCase):
    def setUp(self):
        ...
        self.namecoind_client = BitcoindClient(
            SECRETS['rpc_username'], SECRETS['rpc_password'], port=8336,
            version_byte=52)
...
class ServicesSendTransactionTest(unittest.TestCase):
    def setUp(self):
        ...
        self.bitcoind_client = BitcoindClient(
            SECRETS['rpc_username'], SECRETS['rpc_password'])
        self.bitcoind = create_bitcoind_service_proxy(
            SECRETS['rpc_username'], SECRETS['rpc_password'])

        self.signed_tx = make_send_to_address_tx(
            self.recipient_address, self.send_amount, self.private_key,
            self.chain_com_client)
    ...
    def send_to_address_with_client(self, blockchain_client):
        return send_to_address(
            self.recipient_address, 1000, self.private_key, blockchain_client)
    ...
    def test_build_transaction(self):
        signed_tx = make_send_to_address_tx(
            recipient_address, 1000, private_key, blockchain_client)
        #print signed_tx
...
class ServicesSendOpReturnTransactionTest(unittest.TestCase):
    ...
    def run_op_return_tx_building(self, data, blockchain_client, format='bin'):
        return make_op_return_tx(data, self.private_key, blockchain_client,
                                 fee=self.fee, format=format)
    ...
    def test_hex_op_return_tx(self):
        data = '00'*40
        signed_tx = self.run_op_return_tx_building(data, self.chain_com_client,
                                                   format='hex')
...
class MerkleTest(unittest.TestCase):
    def setUp(self):
        ...
        self.merkle_root = ("4f4c8c201e85a64a410cc7272c77f443d8b8df3289c67af9d"
                            "ab1e87d9e61985e")
